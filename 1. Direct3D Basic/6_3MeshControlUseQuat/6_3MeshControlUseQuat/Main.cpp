...
VOID setWorldMatrix(){
	...
	D3DXQUATERNION quat;
	D3DXMATRIX matRot;
	D3DXQuaternionRotationYawPitchRoll(&quat, fYaw, fPitch, fRoll);
	D3DXMatrixRotationQuaternion(&matRot, &quat);	//convert Quaternion to Matrix
	D3DXMatrixMultiply(&g_matWorld, &matRot, &g_matWorld);

	//Extract Look vector from world matrix
	...
	if (g_bKey['F']){
		g_matWorld._41 += 10*elapsedTime * vLook.x;
		g_matWorld._42 += 10*elapsedTime * vLook.y;
		g_matWorld._43 += 10*elapsedTime * vLook.z;
	}
